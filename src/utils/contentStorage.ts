  ...
  try {
    ...
    if (stored) {
      ...
      if (parsed && typeof parsed === "object" && parsed.hero && parsed.about) {
        // Upgrade or restore to Version 1 (1.3.0) if version is outdated, missing, or projects array is invalid
        if (!parsed.version || parsed.version !== "1.3.0" || !parsed.projects || !Array.isArray(parsed.projects)) {
          console.log("Forcing update to portfolio content Version 1.3.0...");
          try {
